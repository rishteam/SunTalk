...
#define USRefl_ElemList_GetByValue(list, value) list.Get<list.FindValue(value)>()
    ...
    struct TypeInfoBase
    {
        ...
        {
            return detail::DFS_Acc<0>(TypeInfo<type>{}, std::forward<F>(f), VirtualBases().Accumulate(f(std::forward<R>(r), TypeInfo<type>{}, 0), [&](auto &&acc, auto vb) { return std::forward<F>(f)(std::forward<decltype(acc)>(acc), vb, 1); }));
        }
        template <typename F>
        ...
        {
            VirtualBases().ForEach([&](auto vb) { vb.fields.ForEach([&](auto fld) { if constexpr (!fld.is_static && !fld.is_func) std::forward<Func>(func)(std::forward<U>(obj).*(fld.value)); }); });
